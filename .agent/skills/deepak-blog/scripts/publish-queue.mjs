...
if (!existsSync(resolve(process.cwd(), briefPath)) && !existsSync(resolve(process.cwd(), jsonPath))) {
  ...
  console.error(`   Run: node .agent/skills/deepak-blog/scripts/trend-research.mjs --preset deepak-15 --out research-brief.md --json research-brief.json`);
  process.exit(2);
}
...
function excerptFor(q) {
  ...
  const kw = q.keyword;
  const title = q.title;
  const kwNoYear = kw.replace(/\s*2026\s*/g, " ").trim().replace(/\s+/g, " ");
  let s = "";
...
const memoryPath = resolve(process.cwd(), `${skillDir}/memory.md`);
...
for (const q of queue) {
  ...
  const auditScript = resolve(process.cwd(), `${skillDir}/scripts/audit-blog.mjs`);
  const audit = spawnSync("node", [auditScript, "--body", tmpDraft, "--title", title, "--excerpt", excerpt.replace(/\\/g, ""), "--tag", tag, "--out", auditOut], { encoding: "utf8" });
